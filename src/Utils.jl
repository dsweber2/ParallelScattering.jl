...
"""
function calculateSizes(layers::stParallel{K,1}, outputSubsample, Xsize; totalScales = [-1 for i = 1:depth(layers)+1], percentage = 0.9) where {K}
...
function computeAllWavelets(layers, N; outputSubsample = (-1, -1), totalScales = [-1 for i = 1:depth(layers)+1], percentage = 0.9)
    n, q, dataSizes, outputSizes, resultingSize = calculateSizes(layers, outputSubsample, N, totalScales = totalScales, percentage = percentage)
    daughters, ω = computeWavelets(n[1], layers.shears[1]; nScales = -1)
...
function plotAllWavelets(allDaughters)
    return plot(plot([allDaughters[1] sum(allDaughters[1], dims = 2)], legend = false, title = "layer 1"),
        heatmap([allDaughters[1] sum(allDaughters[1], dims = 2)]),
        plot([allDaughters[2] sum(allDaughters[2], dims = 2)], legend = false, title = "layer 2"),
